	...
	Else 
		...
		Case of 
			...
			: (aiLoText16{$viElement}="Array from page")
				...
				$cText:=Split string:C1554(vTextSummary; <>vCR)
			...
			: (aiLoText16{$viElement}="FieldList")
				vTextSummary:=FieldList_json(Table name:C256(curTableNum))+<>vCR+<>vCR+vTextSummary
			...
			: (aiLoText16{$viElement}="Object Map")
				...
				$textAdd:=""
				vTextSummary:=JSON Stringify:C1217($obMap)+<>vCR+<>vCR+vTextSummary
			...
			: (aiLoText16>viTallyMasterArrayElement)  //Field List 
				...
				vTextSummary:=vTextSummary+<>vCR+<>vCR+"/////// Template ///////"+<>vCR+<>vCR+$template
			: (aiLoText16{$viElement}="Fields")  //Field List              
				For ($i; 1; $k)
					$textAdd:=$textAdd+"["+Table name:C256(curTableNum)+"]"+theFields{aFieldLns{$i}}+<>vCR
				End for 
				Case of 
					: ((is4DWriteUser=3) & (eLetterArea>0))
						//**WR INSERT TEXT (eLetterArea;$textAdd)
					Else 
						vTextSummary:=$textAdd+<>vCR+<>vCR+vTextSummary
				End case 
			...
			: (aiLoText16{$viElement}="Data Tags")  //Data
				$TableNum:=String:C10(curTableNum)
				For ($i; 1; $k)
					$textAdd:=$textAdd+Tag_jit_FromNumbers(curTableNum; theFldNum{aFieldLns{$i}})+<>vCR
					// HTML_jitTagMake (1;$TableNum;String(theFldNum{aFieldLns{$i}});theFields{aFieldLns{$i}})+<>vCR
				End for 
			...
			: (aiLoText16{$viElement}="Data_XML")  //Data XML
				$TableNum:=String:C10(curTableNum)
				$textAdd:="<"+Table name:C256(curTableNum)+" DataType="+Txt_Quoted("Table")+">"+<>vCR
				For ($i; 1; $k)
					$textAdd:=$textAdd+Tag_jit_FromNumbers(curTableNum; theFldNum{aFieldLns{$i}})+<>vCR
				End for 
				$textAdd:=$textAdd+"</"+Table name:C256(curTableNum)+">"+<>vCR
			...
			: (aiLoText16{$viElement}="List")  //List
				...
				For ($i; 1; $k)
					$textAdd:=$textAdd+theFields{aFieldLns{$i}}+<>vTab
				End for 
				$textAdd:=$textAdd+<>vCR+"<!--  _jit_begin_"+String:C10(curTableNum)+"_1jj  --> "+<>vCR
				For ($i; 1; $k)
					$textAdd:=$textAdd+Tag_jit_FromNumbers(curTableNum; theFldNum{aFieldLns{$i}})+<>vTab
				End for 
				$textAdd:=$textAdd+<>vCR+"<!--  _jit_end_jj  -->"+<>vCR
			...
			: (aiLoText16{$viElement}="Lists_XML")  //List XML
				...
				$TableNum:=String:C10(curTableNum)
				$textAdd:="<?xml version="+Txt_Quoted("1.0")+" encoding="+Txt_Quoted("UTF-8")+"?>"+<>vCR
				$textAdd:=$textAdd+"<"+Table name:C256(curTableNum)+"List DataType="+Txt_Quoted("List")+">"+<>vCR
				$textAdd:=$textAdd+<>vCR+"_jit_begin_"+String:C10(curTableNum)+"_1jj"+<>vCR
				$textAdd:=$textAdd+"<"+Table name:C256(curTableNum)+" DataType="+Txt_Quoted("Table")+">"+<>vCR
				For ($i; 1; $k)
					$textAdd:=$textAdd+Tag_jit_FromNumbers(curTableNum; theFldNum{aFieldLns{$i}})+<>vCR
				End for 
				$textAdd:=$textAdd+<>vCR+"</"+Table name:C256(curTableNum)+">"+<>vCR
				$textAdd:=$textAdd+<>vCR+"_jit_end_jj"+<>vCR+"</"+Table name:C256(curTableNum)+"List>"+<>vCR
				vTextSummary:=$textAdd+<>vCR+<>vCR+vTextSummary
