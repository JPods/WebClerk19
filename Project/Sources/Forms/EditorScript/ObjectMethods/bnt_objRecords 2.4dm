...
$error:=AL_GetSelect(eExportFlds; aFieldLns)



C_LONGINT:C283($incRay; $cntRay)
C_LONGINT:C283($processNum)
C_TEXT:C284($tableName_t; $fieldList)
C_POINTER:C301($ptTable)
$ptTable:=Table:C252(curTableNum)
$tableName_t:=Table name:C256(curTableNum)
$cntRay:=Size of array:C274(<>aPrsName)
$processNum:=0
For ($incRay; 1; $cntRay)
	If (Position:C15($tableName_t; <>aPrsName{$incRay})>0)
...
End if 
C_TEXT:C284($tableName_t; $fieldList; $tallyMasterName; $fieldValue)
C_TEXT:C284($vtFieldList)
$vtFieldList:=SE_jsonFieldList($tableName_t; Size of array:C274(aFieldLns))
C_OBJECT:C1216($veSelection)
$veSelection:=API_SelectionToObject($tableName_t)
$jsondata:=API_EntityToText($veSelection; $vtFieldList)
//end if
vTextSummary:=vTextSummary+(Num:C11(vTextSummary#"")*(<>vCR*3))+$jsondata
