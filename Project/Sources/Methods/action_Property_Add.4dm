...
If (($event.code=On Clicked:K2:4) | ($event.code=On Double Clicked:K2:5))
	
	$vAddMenu:=Create menu:C408
	If (Not:C34(Form:C1466.entityIsReadOnly))
		...
		If ($collection_Sel.length>0)
			...
			APPEND MENU ITEM:C411($vAddMenu; Util25_Get_LocalizedMessage("Modify Property"; $collection_Cur.Property))
	...
	If (($choice#"") & ($choice#"NOCLICK"))
		...
		If ($choice="MOD")  //We want to modify the selected property
			$form.case:="MOD"
			$form.title:=Util25_Get_LocalizedMessage("Modifying Property"; $collection_Cur.Property)
			$form.type:=Util25_ConvertTypeToText($collection_Cur.Type)
		...
		If (OK=1)
			...
			Else 
				...
				$valueType:=Util25_ConvertText2Type($form.type)
				$index:=-5
			End if 
			...
			End case 
			$name:=Util25_FilterPropertyName($form.propertyName)
			If ($form.case="ADD")
				Repeat 
			...
			$newObject.Type:=Util25_ConvertText2Type($form.type)
			If ($form.case="ADD")
				$collection_Main.push($newObject)
			End if 
		End if 
		$collection_Main:=$collection_Main
		Form:C1466["data_"]:=Form:C1466["data_"]
	End if 
End if 
