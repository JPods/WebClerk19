...
Else 
	$incustomerID:=""
End if 
$lOrderNum:=IE_OrdFindFieldPtrInArray((->[Order:3]orderNum:2); ->aptOrderFieldPointers; ->atOrderImport{$lIndice})->
...
Case of 
	: (Records in selection:C76([Customer:2])=1)
		...
		[Order:3]orderNum:2:=$OrderNum
		[Order:3]customerID:1:=$customerID
		
		[Order:3]commentProcess:12:=vtImportUniqueTag+"\r"+[Order:3]commentProcess:12  //see IE_OrdImportCustomers
		...
		If ((vsCCNum#"") & (vsCCExpire#""))
			...
			[Payment:28]orderNum:2:=[Order:3]orderNum:2
			...
			[Payment:28]dateReceived:10:=[Order:3]dateOrdered:4
