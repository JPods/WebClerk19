...
Else 
	...
	If ($theSource#"")
		...
		If (Records in selection:C76([TallyResult:73])=1)
			...
			$numCols:=[TallyResult:73]longInt2:8
			Case of 
	...
	If (OK=1)
		...
		//MULTI SORT ARRAY (aTmpLong1;>;aTmpReal3;<;aTmpReal1;aTmpReal2;aTmpReal4)
		SORT ARRAY:C229(aTmpLong1; aTmpReal3; aTmpReal1; aTmpReal2; aTmpReal4; >)
		...
		For ($i; 1; $k)  //do each lable
			...
			If (($pT>0) | ($pP>0))
				...
				C_TEXT:C284($grpCmd)
				$doNewPage:=($keyYPoint+aTmpReal4{$i}>$pageHeight)
				Case of 
					: (($countLabels>$labelsInCol) | ($doNewPage))
						$doNewPage:=False:C215
						$countLabels:=0
						$countCols:=$countCols+1
						$keyXPoint:=$maxX+$xIncrement  //+aTmpReal3{1}
						...
						If ($countCols>$numCols)
							...
							sumDoc:=EI_UniqueDoc(Storage:C1525.folder.jitExportsF+$temName)
							$countCols:=1
							SEND PACKET:C103(sumDoc; vText1)
				...
				If ($labelOffSet>0)  //lable on top of stamp//$aXShell{$incRay}  changed from $xShellOne
					For ($incRay; 1; $cntRay)
						$aBoxX{$incRay}:=$keyXPoint+($vWideShell/2)-(aTmpReal3{$i}/2)+$aBoxX{$incRay}-aTmpReal1{$i}+$xShellOne  //+5   February 16, 2000
				...
				Else 
					...
					For ($incRay; 1; $cntRay)
						$aBoxX{$incRay}:=$keyXPoint+($vWideShell/2)-(aTmpReal3{$i}/2)+$aBoxX{$incRay}-aTmpReal1{$i}+$xShellOne  //+3   February 16, 2000
				...
				If (aTmpReal4{$i}<=$yIncrement)
					C_LONGINT:C283($thisYStep)
					$thisYStep:=1
				Else 
					TRACE:C157
					$thisYStep:=((aTmpReal4{$i}+$vTallShell+($yIncrement*0.5))\$yIncrement)+1
				End if 
				$keyYPoint:=$keyYPoint+($yIncrement*$thisYStep)
				//If ($cntRay>0)
		...
		If ($setAppleEvents)
			...
			ALERT:C41("AppleScript disabled - method Qx_PlaceTopRiders")
