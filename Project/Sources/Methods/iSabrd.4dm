...
If (<>iSabrd=1)  // & (Current user="bill.james"))
	If (<>iLoText5="")
		<>iLoText5:=Request:C163("Define iSabrdGroup for this import")
	End if 
	...
	If ($p>0)
		...
		If (False:C215)  // QQQ work duplicates with ORDA
			C_TEXT:C284($firstName; $lastName; $zip; $class; $school; $graduationname)
			// rework conflicts with entities
			$lastName:=$custNew_o.nameLast
			$firstName:=$custNew_o.nameFirst
			If ($graduationname#"")
				$custExisting_o:=ds:C1482.Customer.query(" sICCode= :1 | nameLast = :2 & nameFirst = :3"; $namegraduation; $lastName; $firstName)
			Else 
				$custExisting_o:=ds:C1482.Customer.query(" nameLast = :1 & nameFirst = :2"; $lastName; $firstName)
			End if 
			If ($custExisting_o.length=0)
				KeyWordsMake($custNew_o)
			Else 
				// 
				BEEP:C151
