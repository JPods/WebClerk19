...
If ($tableName#"")
	...
	If ($tableNum>0)
		vResponse:="No valid Record for Table: "+$tableName
		...
		If ($recordNum>=0)  //must have at least one record
			...
			If (Locked:C147(Table:C252($tableNum)->))
				vResponse:="Record was locked by another process.  Change will not be saved."
			Else 
				vResponse:="Record available for changes."
			End if 
