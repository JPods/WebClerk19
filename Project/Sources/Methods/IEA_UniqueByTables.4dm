...
Case of 
	...
	: ($1=(->[Order:3]))
		$doSave:=IEA_UniqueThisTable(->[Order:3]idNum:2)
	: ($1=(->[Item:4]))
	...
	: ($1=(->[Invoice:26]))
		$doSave:=IEA_UniqueThisTable(->[Invoice:26]idNum:2)
	: ($1=(->[InventoryStack:29]))
	...
	: ($1=(->[PO:39]))
		$doSave:=IEA_UniqueThisTable(->[PO:39]idNum:5)
	: ($1=(->[Proposal:42]))
		$doSave:=IEA_UniqueThisTable(->[Proposal:42]idNum:5)
	: ($1=(->[Usage:5]))
