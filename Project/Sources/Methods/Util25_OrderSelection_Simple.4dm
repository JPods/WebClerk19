...
Util25_GetPropertyTypePictures($formData; $colOK4Sort; "SORT")
	...
	Case of 
		: ($zeProperty.kind="storage")
			If ($colOK4Sort.indexOf($zeProperty.type)>-1)  //We can use it for sorting
				$formData.propertyList.push(New object:C1471("name"; $zeProperty.name; "type"; $zeProperty.type; "localName"; Util25_Get_LocalizedMessage($zeProperty.name)))
			End if 
...
SET WINDOW TITLE:C213(Util25_Get_LocalizedMessage("Order_By"; $dataClass.getInfo().name); $w)
