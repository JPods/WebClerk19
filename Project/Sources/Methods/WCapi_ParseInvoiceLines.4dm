...
// Date and time: 10/02/21, 23:21:39
// ----------------------------------------------------
...
Else 
	...
	For each ($voIncomingLine; $obPayload.lines)
		...
		If ($voIncomingLine.id=Null:C1517)  // new line
			...
			$voExistingLine.invoiceNum:=$voRecPrimary.invoiceNum
			...
			For each ($value; $voIncomingLine)
				Case of 
					: ($value="ParentInvoice")
						...
						// sets the invoiceNum to null
					: ($value="invoiceNum")
						// restricted
					: (($value="id") | ($value="invoiceNum") | ($value="orderNum") | ($value="idNum") | ($value="customerID"))
						// restricted
					: ($value="itemNum")
						// restricted
					Else 
	...
	$obOutbound.errors:=New collection:C1472
	$obOutbound.errors.push(voState.errors)
