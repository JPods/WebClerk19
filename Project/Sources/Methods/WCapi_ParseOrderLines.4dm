...
// Date and time: 01/18/21, 21:18:51
// ----------------------------------------------------
// Method: WCapi_ParseOrderLines
// Description
// 
//
// Parameters
...
$voRecPrimary:=ds:C1482.Order.query("id= "+$vtUUIDKey).first()
If ($voRecPrimary=Null:C1517)
	vResponse:="Error: No Order Record for "+$vtUUIDKey
Else 
	...
	For each ($voIncomingLine; $obPayload.lines)
		...
		If ($voIncomingLine.id=Null:C1517)  // new line
			$voExistingLine:=ds:C1482.OrderLine.new()
			$voExistingLine.orderNum:=$voRecPrimary.orderNum
		...
		Else 
			$voExistingLine:=ds:C1482.OrderLine.query("id = :1"; $voIncomingLine.id).first()
			...
			For each ($value; $voIncomingLine)
				Case of 
					...
					: ($value="orderNum")
						// restricted
					: (($value="id") | ($value="orderNum") | ($value="idNum") | ($value="customerID"))
						// restricted
					: ($value="itemNum")
						// restricted
					Else 
	...
	// dORDA
	ParseOrderRecord($voRecPrimary.orderNum)
