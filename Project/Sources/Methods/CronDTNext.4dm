...
// Date and time: 03/07/12, 23:29:07
// ----------------------------------------------------
// Method: CronDTNext
// Description
// 
...
//Script:   CronJobs dtNextStart 20111115
...
C_TEXT:C284($1; $cronText)
$cronText:=$1
//$cronText:=[TallyMaster]ItemNum
...
If (False:C215)
	$cronText1:=""
	$cronText1:=$cronText1+"Current Minute "+vtMinute+"\r"
	$cronText1:=$cronText1+"Current Hour "+vtHour+"\r"
	$cronText1:=$cronText1+"Current Day "+vtDay+"\r"
	$cronText1:=$cronText1+"Current Month "+vtMonth+"\r"
	$cronText1:=$cronText1+"Current Weekday "+vtWeekday+" = "+atDayOfWeek{viWeekday}+"\r"
	$cronText1:=$cronText1+"Current Year "+vtYear+"\r"
	ALERT:C41($cronText1)
	
	//$cronText:="*"+vTab+"*"+vTab+"*"+vTab+"*"+vTab+"*"
	//$cronText:="*"+vTab+"*"+vTab+"1"+vTab+"9"+vTab+"*"
	//$cronText:=Request("Enter Cron Date Time * * * * *")
	//Alert($cronText)
End if 

//vText := "0/5 9-17 * * 1-5 *"
//vText:=[TallyMaster]ItemNum//Alert(vText)

viPointer:=1
vi2:=Length:C16($cronText)
For (vi1; 1; vi2)
	
	If (($cronText[[vi1]]=vTab) | ($cronText[[vi1]]=" "))
		viPointer:=viPointer+1
	Else 
		Case of 
				
			: (viPointer=1)  //minute (0-59)
				vtCronMinute:=vtCronMinute+$cronText[[vi1]]
				
			: (viPointer=2)  //hour (0-23)
				vtCronHour:=vtCronHour+$cronText[[vi1]]
				
			: (viPointer=3)  //day of the month (1-31)
				vtCronDay:=vtCronDay+$cronText[[vi1]]
				
			: (viPointer=4)  //month (1-12)
				vtCronMonth:=vtCronMonth+$cronText[[vi1]]
				
			: (viPointer=5)  //day of the week (1-7) (Sunday = 1) 4D uses 1-7 as Sunday - Saturday
				vtCronWeekday:=vtCronWeekday+$cronText[[vi1]]
				
			: (viPointer=6)  //Year 4 digit match for one time scripts
				vtCronYear:=vtCronYear+$cronText[[vi1]]
...
For (vi1; 1; vi2)
	$cronText:=atCronMinute{vi1}
	viLength:=Length:C16($cronText)
	viPosition:=Position:C15("-"; $cronText)
	If ((viPosition#0) & (viPosition#1) & (viPosition#viLength))  //delimiter must exist and can't be the first or last character
		$cronText1:=Substring:C12($cronText; 1; viPosition-1)
		$cronText2:=Substring:C12($cronText; viPosition+1; (viLength-viPosition))
		//Alert($cronText1)
		//Alert($cronText2)
		viIndex:=Num:C11($cronText1)
		viEnd:=Num:C11($cronText2)
		While (viIndex<=viEnd)
...
For (vi1; 1; vi2)
	$cronText:=atCronWeekday{vi1}
	viLength:=Length:C16($cronText)
	viPosition:=Position:C15("L"; $cronText)
...
If (False:C215)
	$cronText1:=""
	$cronText1:=$cronText1+"Cron Minute "+vtCronMinute+" "+vtCronMinuteAll+"\r"
	$cronText1:=$cronText1+"Cron Hour "+vtCronHour+" "+vtCronHourAll+"\r"
	$cronText1:=$cronText1+"Cron Day "+vtCronDay+" "+vtCronDayAll+"\r"
	$cronText1:=$cronText1+"Cron Month "+vtCronMonth+" "+vtCronMonthAll+"\r"
	$cronText1:=$cronText1+"Cron Weekday "+vtCronWeekday+" "+vtCronWeekdayAll+"\r"
	$cronText1:=$cronText1+"Cron Year "+vtCronYear+" "+vtCronYearAll+"\r"
	ALERT:C41($cronText1)
End if 
...
//[TallyMaster]RealName3:=(String(vdDate;7)+" - "+String(vhmyTime;2))
...
//[TallyMaster]DTNextStart:=vdt
//[TallyMaster]DateRevision:=vdDate
