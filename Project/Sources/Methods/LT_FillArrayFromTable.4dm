...
SELECTION TO ARRAY:C260([LoadTag:88]; aLtTagRecordID; [LoadTag:88]idNum:1; aLtLoadTagID; [LoadTag:88]documentid:17; aLtOrderNum; [LoadTag:88]poNum:18; aLtPoNum; [LoadTag:88]invoiceNum:19; aLtinvoiceNum; [LoadTag:88]status:6; aLtTagStatus; [LoadTag:88]trackingid:7; aLttrackID)
...
C_LONGINT:C283($i; $k)
For ($i; 1; $k)
	jDateTimeRecov($aLtStage{$i}; ->aLtStage{$i})
	jDateTimeRecov($aLtShipOn{$i}; ->aLtShipOn{$i})
	jDateTimeRecov($aLtCustoms{$i}; ->aLtCustoms{$i})
	jDateTimeRecov($aLtExpected{$i}; ->aLtExpected{$i})
End for 
