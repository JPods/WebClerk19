...
If (Count parameters:C259<2)
	...
	If (ok=1)
		...
		If (ok=1)
			...
			$out_o:=XML_ToObject($XMLroot_t; "parse")  //;->$Xpath_at)
		End if 
		...
		If (False:C215)
			SET TEXT TO PASTEBOARD:C523(JSON Stringify:C1217($out_o))
		End if 
...
Else 
	...
	If ($nbElementsEnfants_l>0)
		...
		For ($i_l; 1; $nbElementsEnfants_l)
			...
			$objEnfant_o:=XML_ToObject($refXMLenfant_at{$i_l}; "parseSub")
