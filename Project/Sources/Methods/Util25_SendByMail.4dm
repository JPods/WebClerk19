...
$subject:=Util25_SendByMail_Parse($settings.Company.Subject; $invoice; $client; $settings)
$textBody:=Util25_SendByMail_Parse($settings.Company.Body; $invoice; $client; $settings)
$attachment:=$pdf
...
Else 
	If ($status.status=0)
		...
		ALERT:C41(Util25_Get_LocalizedMessage("CheckNo"; $tmess{1}))  //"an error occurred: "+$tmess{1})
	Else 
		ALERT:C41(Util25_Get_LocalizedMessage("CheckNo"; $status.statusText))
	End if 
End if 
