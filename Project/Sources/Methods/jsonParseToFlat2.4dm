...
// This is wrong keep as reference until 
// 2018-12-15
C_OBJECT:C1216($1)
...
For ($vi1; 1; Size of array:C274($atNames))
	...
	If ($viType=Object array:K8:28)  //  39) 
		...
		ARRAY OBJECT:C1221($aObjectElement; 0)  // extract an object array into its own array
		C_TEXT:C284($working)
		OB GET ARRAY:C1229($voBatch; $atNames{$vi1}; $aObjectElement)
		C_LONGINT:C283($incRay; $cntRay)
		$cntRay:=Size of array:C274($aObjectElement)
		For ($incRay; 1; $cntRay)
			C_TEXT:C284($property)
			$viType:=OB Get type:C1230($aObjectElement{$incRay}; $property)
			If ($viType=Object array:K8:28)  //  39) 
				jsonParsingToFlat($aObjectElement{$incRay})
