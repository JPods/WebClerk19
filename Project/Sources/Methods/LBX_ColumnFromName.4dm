...
// Method: LBX_ColumnFromName
// Description 
// Parameters
// ----------------------------------------------------

// changed from LBName to dataClassName so they can be more easily used in multiple Listboxes
#DECLARE($dataClassName : Text; $fieldName_t : Text; $columnAdder_t : Text)->$column_o : Object
If ($fieldName_t="")
	// not valid name
Else 
	If (ds:C1482[$dataClassName][$fieldName_t]=Null:C1517)
		// not valid name
	Else 
		$dataClass:=ds:C1482[$dataClassName]
		$column_o:=New object:C1471
		$column_o.name:="Column_"+$dataClassName+"_"+$fieldName_t+"_"+$columnAdder_t
		$column_o.dataSource:="This."+$fieldName_t
		$column_o.fieldType:=$dataClass[$fieldName_t].fieldType
		
		$column_o.header:=New object:C1471("name"; "Header_"+$dataClassName+"_"+$fieldName_t+"_"+$columnAdder_t; \
			"text"; $fieldName_t)
		
		$column_o.footer:=New object:C1471("name"; "Footer_"+$dataClassName+"_"+$fieldName_t+"_"+$columnAdder_t)
