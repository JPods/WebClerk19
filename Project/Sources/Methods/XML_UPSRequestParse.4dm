//%attributes = {}
//After	Build	DateRevision	DTNextRun	DTNextStart	Duration	FieldNum	HeadAdder	ItemNum	LongName1	LongName2	Name	Path	Profile1	//Profile2	ProfileName1	ProfileName2	ProfileName3	Publish	Purpose	RealName1	RealName2	RealName3	RealName4	revisionID	Script	siteID	//Status	TableNum	UniqueID
//			00/00/00 00:00:00	00/00/00 00:00:00	0	0					UPS_ShipOnDate		Orders		Table			3	Scripts					0	"//Script UPS_ShipOnDate 20111209
...
If (False:C215)  //skip for testing only, begin
	...
	If (Records in selection:C76([TallyMaster:60])=1)
		...
		vText4:=TagsToText([TallyMaster:60]script:9)
		vText:=BLOB to text:C555(blobPageOut; UTF8 text without length:K22:17)  //  -> vText
		...
		TEXT TO BLOB:C554(HTTPSendText; HTTP_Data; UTF8 text without length:K22:17; *)  // convert text to blob for sending
		
		// Send the request
		
		//vlError:=WC_Request ("POST")   //### jwm ### does not work from script
		
		//vxResponse:=HTTP_IncomingBlob
		//SET BLOB SIZE(HTTP_IncomingBlob;0)
		//
		...
		vtHeaders:=vtHeaders+BLOB to text:C555(HTTP_Data; UTF8 text without length:K22:17)  //### jwm ###
		
		// Convert the headers and data to a blob
		TEXT TO BLOB:C554(vtHeaders; vxOutGoingBlob; UTF8 text without length:K22:17)
		...
		HTTPClient_Response:=BLOB to text:C555(vxResponse; UTF8 text without length:K22:17)
		
		[TallyMaster:60]after:7:=HTTPClient_Response
		SAVE RECORD:C53([TallyMaster:60])
	End if   //skip for testing only, end
End if   //skip for testing only, end

myDoc:=Open document:C264("")
<>vEoF:=Get document size:C479(document)+1  // ### jwm ### 20160714_1147 is plus 1 needed ?
RECEIVE PACKET:C104(myDoc; HTTPClient_Response; <>vEoF)
