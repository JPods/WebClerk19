	...
	Case of 
		: ($what="Entity")
			If (Form:C1466.recordCanBeSaved)
				$text:=Util25_Get_LocalizedMessage("delete this Entity")
				...
				If (OK=1)
					...
					Case of   //Handling of special cases
						: (Form:C1466.dataClassName="Invoices")  //To delete an invoice, it's necessary to delete invoice lines first
							$subsel2Kill:=Form:C1466.editEntity.Invoices_to_Invoice_Lines
						...
						Else 
							Case of 
								: ($status.status=dk status locked:K85:21)  //This case should never happen in case of Pessimistic Locking!
									ALERT:C41(Util25_Get_LocalizedMessage("Recordinuse"; $status.lockInfo.user_name))
								...
								: ($status.status=dk status serious error:K85:22)
									ALERT:C41(Util25_Get_LocalizedMessage("Something strange"; $status.lockInfo.errors.text.join(Char:C90(Carriage return:K15:38))))
		...
		: ($what="Selection")
			...
			$text:=Util25_Get_LocalizedMessage("deleteSelection"; String:C10($n))
			CONFIRM:C162($text; Get localized string:C991("Delete it"); Get localized string:C991("Cancel"))
			If (OK=1)
				...
				Case of   //Handling of special cases
					: (Form:C1466.dataClassName="Invoices")  //To delete an invoice, it's necessary to delete invoice lines first
						$subsel2Kill:=Form:C1466.selectedSubset.Invoices_to_Invoice_Lines
						$lockedSelection:=$subsel2Kill.drop(dk stop dropping on first error:K85:26)
						$fl_Stop:=($lockedSelection.length>0)
				...
				Else 
					...
					If ($lockedSelection.length=0)  //The delete action is successful, all the entities have been deleted
						VALIDATE TRANSACTION:C240
						ALERT:C41(Util25_Get_LocalizedMessage("You have dropped"; String:C10($n); Form:C1466.dataClassName))  //The dropped entity selection remains in memory
