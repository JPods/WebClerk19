			...
			: ($zeProperty.kind="storage")
				...
				Case of 
						...
						For each ($path; $zePaths)
							Case of 
								: ($path="@[]")
									...
									$qryPropertyList.push(New object:C1471("name"; $zeProperty.name+"."+$path; "type"; "array"; "localName"; Util25_Get_LocalizedMessage($zeProperty.name)+"."+$path))
								...
								Else 
									$qryPropertyList.push(New object:C1471("name"; $zeProperty.name+"."+$path; "type"; "variant"; "localName"; Util25_Get_LocalizedMessage($zeProperty.name)+"."+$path))
					...
					: ($colOK2Use.indexOf($zeProperty.type)>-1)  //We can use it for searching
						$qryPropertyList.push(New object:C1471("name"; $zeProperty.name; "type"; $zeProperty.type; "localName"; Util25_Get_LocalizedMessage($zeProperty.name)))
				...
				For each ($relatedProperty; $zeRelatedDataclass)
					...
					If ($zeRelatedProperty.kind="storage")
						If ($colOK2Use.indexOf($zeProperty.type)>-1)
							$qryPropertyList.push(New object:C1471("name"; $zeProperty.name+"."+$zeRelatedProperty.name; "type"; $zeRelatedProperty.type; "localName"; Util25_Get_LocalizedMessage($zeProperty.name)+"."+Util25_Get_LocalizedMessage($zeRelatedProperty.name)))
						End if 
					End if 
				End for each 
