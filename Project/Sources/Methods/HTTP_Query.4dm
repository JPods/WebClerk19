...
// Date and time: 01/04/10, 17:31:10
// ----------------------------------------------------
// Method: HTTP_Query
// Description
...
If ($tableNum>0)
	...
	If ($sizeRay>0)
		...
		If ($tableNum=-1)
			...
			vResponse:="customerID not passed."
		Else 
			...
			If ($theQuery#"")
				...
				vResponse:="Query Executed. Records found: "+String:C10(Records in selection:C76(Table:C252($tableNum)->))
			Else 
				vResponse:="Query not correctly defined."
				$doPage:=WC_DoPage("Error.html"; "")
			End if 
		End if 
		Case of 
			: (Records in selection:C76(Table:C252($tableNum)->)=1)
				vResponse:=""
			...
			: (vResponse="Query Executed@")
				$jitPageOne:=WCapi_GetParameter("jitPageList"; "")
				$doPage:=WC_DoPage($tableName+"List.html"; $jitPageOne)
...
Else 
	$doPage:=WC_DoPage("Error.html"; "")
	vResponse:="TableName: "+$tableName+" or FieldName: "+$fieldName1+" not correct"
End if 
