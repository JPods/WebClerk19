...
Case of 
	: ([RemoteUser:57]tableNum:9=2)
		...
		If (Records in selection:C76([Customer:2])=1)
			...
			If ([Customer:2]mfrLocationid:67<-10000)
				QUERY:C277([zManufacturerTerm:111]; [zManufacturerTerm:111]customerID:1=[Customer:2]customerID:1; *)
				QUERY:C277([zManufacturerTerm:111];  & [zManufacturerTerm:111]active:3=True:C214)
				If (Records in selection:C76([zManufacturerTerm:111])=1)
					RemoteUsers_SetVars
				End if 
			End if 
			[RemoteUser:57]dtLastVisit:5:=DateTime_DTTo
		Else 
	...
	: ([RemoteUser:57]tableNum:9=Table:C252(->[Vendor:38]))
		...
		End if 
		SAVE RECORD:C53([RemoteUser:57])
	: ([RemoteUser:57]tableNum:9=Table:C252(->[zManufacturerTerm:111]))
		QUERY:C277([Customer:2]; [Customer:2]customerID:1=[RemoteUser:57]customerID:10)
		QUERY:C277([zManufacturerTerm:111]; [zManufacturerTerm:111]customerID:1=[Customer:2]customerID:1)
		If (Records in selection:C76([zManufacturerTerm:111])=1)
			[EventLog:75]customerRecNum:8:=Record number:C243([Customer:2])
