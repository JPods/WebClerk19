...
// Date and time: 01/19/18, 14:07:58
// ----------------------------------------------------
// Method: FormatScriptOnClipboard
// Description
// 
//
// Parameters
...
ARRAY TEXT:C222($atText; 0)
TextToArray(vText; ->$atText; "\n"; True:C214)


viStart:=Find in array:C230($atText; "@<declarations>@")
viEnd:=Find in array:C230($atText; "@</declarations>@")
viStart:=viStart+1
viHowMany:=viEnd-viStart


If ((viStart>0) & (viEnd>0) & (viHowMany>0))
	DELETE FROM ARRAY:C228($atText; viStart; viHowMany)
	vText:=""
	For (i; 1; Size of array:C274($atText))
		vText:=vText+$atText{i}+"\n"
	End for 
End if 
...
ALERT:C41("Text copied to Clipboard")
...
vText1:=Replace string:C233(vText1; "\r\n"; "\r")
vText1:=Replace string:C233(vText1; "\r"; "\n")
...
vText1:=Tx_ConvertScripts(vText1)  // convert script
