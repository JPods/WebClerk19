...
If (Locked:C147([ItemSerial:47]))
	//CREATE RECORD([zzzTempRec])
	//[zzzTempRec]id:=Table(->[ItemSerial])
	//[zzzTempRec]idU:=-Record number([ItemSerial])  //<0 delete orginal
	//DUPLICATE RECORD([ItemSerial])
	//SAVE RECORD([ItemSerial])
	//[zzzTempRec]data:=Record number([ItemSerial])
	//SAVE RECORD([zzzTempRec])
	//UNLOAD RECORD([zzzTempRec])
End if 
[ItemSerial:47]status:8:="Iv"
[ItemSerial:47]customerID:9:=[Invoice:26]customerID:3
[ItemSerial:47]dateDocument:13:=[Invoice:26]dateShipped:4  //[Sale]TransactionDate
[ItemSerial:47]invoiceNum:10:=[Invoice:26]idNum:2  //[Sale]salesNameID
[ItemSerial:47]salesLnRefid:11:=aiLineNum{$2}
If (([ItemSerial:47]dateDocument:13#!00-00-00!) & ([ItemSerial:47]dateReceived:12#!00-00-00!))
	[ItemSerial:47]daysOnPlan:17:=[ItemSerial:47]dateDocument:13-[ItemSerial:47]dateReceived:12
Else 
