...
If ($oCust#Null:C1517)
	...
	For each ($oLedger; $oSel)
		...
		// find a better way to manage locked records
		// UpdateWithResources by: Bill James (2023-01-03T06:00:00Z)
		//$oGarbage:=ds.zzzGarbage.query("tableNum = :1 & id = :2"; $tableNum; $oLedger.id)
		//If ($oGarbage=Null)  // confirm this ledger was not locked during a .drop effort
		Case of 
			: (($curDate+30)<$oLedger.dateDue)  //&($oLedger.CreatingDocType#"Pay"))//future
				$future:=$future+$oLedger.unAppliedValue
				//ConsoleLog("Future\t"+String($oLedger.DocRefID)+"\t"+String(Current Date)+"\t"+String($oLedger.DateDue)+"\t"+String(Current Date - $oLedger.DateDue))
			: ($curDate<$oLedger.dateDue)  // NOT PAST DUE //|($oLedger.CreatingDocType="Pay"))//current
				$dueNow:=$dueNow+$oLedger.unAppliedValue
				//ConsoleLog("Due Now\t"+String($oLedger.DocRefID)+"\t"+String(Current Date)+"\t"+String($oLedger.DateDue)+"\t"+String(Current Date - $oLedger.DateDue))
				
			: (($curDate-30)<$oLedger.dateDue)  //> zero < 30
				$past30:=$past30+$oLedger.unAppliedValue
				//ConsoleLog("Past 30\t"+String($oLedger.DocRefID)+"\t"+String(Current Date)+"\t"+String($oLedger.DateDue)+"\t"+String(Current Date - $oLedger.DateDue))
				
			: (($curDate-60)<$oLedger.dateDue)  //> 30 < 60
				$past60:=$past60+$oLedger.unAppliedValue
				//ConsoleLog("Past 60\t"+String($oLedger.DocRefID)+"\t"+String(Current Date)+"\t"+String($oLedger.DateDue)+"\t"+String(Current Date - $oLedger.DateDue))
				
			Else   //: (($curDate-90)>$oLedger.DateDue)//over 60
				$past90:=$past90+$oLedger.unAppliedValue
				//ConsoleLog("Past 90\t"+String($oLedger.DocRefID)+"\t"+String(Current Date)+"\t"+String($oLedger.DateDue)+"\t"+String(Current Date - $oLedger.DateDue))
