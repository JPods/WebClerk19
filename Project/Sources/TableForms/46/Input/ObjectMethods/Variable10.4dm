...
Case of 
	...
	: (Form event code:C388=On Load:K2:1)
		...
		APPEND TO ARRAY:C911(aSREdits; "Set Height")
		APPEND TO ARRAY:C911(aSREdits; "Set Width")
	...
	: (aSREdits>2)
		Case of 
			...
			: (aSREdits{aSREdits}="Set Width")
				SREActionSetWidth
			: (aSREdits{aSREdits}="Set Height")
				SREActionSetHeight
			...
			Else 
				...
				//QUERY([TallyMaster];&[TallyMaster]Publish<=Storage.userPrefs.securityLevel)
				If (Records in selection:C76([TallyMaster:60])>0)
					FIRST RECORD:C50([TallyMaster:60])
					ExecuteText(0; [TallyMaster:60]script:9; [TallyMaster:60]name:8)
					//SET TEXT TO PASTEBOARD([TallyMaster]Script)
