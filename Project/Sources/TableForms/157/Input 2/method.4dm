...
Else 
	If ($formEvent=On Load:K2:1)
		If (vHere>1)  //coming from another table's input layout
			If (Is new record:C668([QQQResult:157]))
	...
	//
	C_LONGINT:C283($formEvent)
	$formEvent:=iLoProcedure(->[QQQResult:157])
	//
	$doMore:=False:C215
	Case of 
		: (iLoRecordNew)  //set in iLoProcedure only once, new record
			If ([QQQResult:157]idUniqueObjective:2=0)
				[QQQResult:157]idUniqueObjective:2:=[QQQObjective:145]idUnique:4
			End if 
			$doMore:=True:C214
