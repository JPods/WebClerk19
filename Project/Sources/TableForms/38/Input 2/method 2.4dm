	...
	Else 
		If ($formEvent=On Load:K2:1)  //|(booPreNext))//(Before)
			...
			Else 
				...
				jrelateClrFiles(0)
				jFormatPhone(->[Vendor:38]phone:10; ->srPhone; ->[Vendor:38]fax:13; ->[Vendor:38]phoneCell:82)
			End if 
			jNxPvButtonSet
		End if 
		...
		If ($curRecNum>-1)
			If (<>aLastRecNum{Table:C252(ptCurTable)}#$curRecNum)  //|($formEvent=On Load))))
				...
				End if 
				jNxPvButtonSet
				jFormatPhone(->[Vendor:38]phone:10; ->srPhone; ->[Vendor:38]fax:13)
