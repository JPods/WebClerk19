...
Else 
	...
	If ($doMore)  //action for the form regardless of new or existing record
		...
		DateTime_DTFrom([RemoteUser:57]dtLastVisit:5; ->vDate1; ->vTime1)
		DateTime_DTFrom([RemoteUser:57]dtCreated:8; ->vDate2; ->vTime2)
		...
		vi1:=0
		Case of 
			: (([RemoteUser:57]tableNum:9=2) | ([RemoteUser:57]tableNum:9=(Table:C252(->[zManufacturerTerm:111]))))
