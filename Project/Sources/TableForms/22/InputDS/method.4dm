...
Else 
	...
	Case of 
		: (iLoRecordNew)  //set in iLoProcedure only once, new record
			[ItemXRef:22]itemNum:1:=[Item:4]itemNum:1
			...
			OBJECT SET ENTERABLE:C238([ItemXRef:22]itemNum:1; False:C215)
	...
	If ($doMore)  //action for the form regardless of new or existing record
		If (Record number:C243([Order:3])>-1)
			DateTime_DTFrom([Order:3]dtProdRelease:56; ->vDate1; ->vTime2)
		End if 
	...
	booAccept:=(([ItemXRef:22]itemNumXRef:2#"") & ([ItemXRef:22]itemNum:1#""))
End if 
