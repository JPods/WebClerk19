...
Case of 
	...
	: ($obFormEvent.code=On Drop:K2:12)
		...
		// LBDocument_ent:=Form.obSel.copy()
		
		// should already be loaded
		// LBDocument_ent:=ds.Document.query("customerID = :1 and tableNum = :2"; [Customer]customerID; 2)
		...
		For ($inc; 1; $cnt)
			...
			$obRec.idNumTask:=[Proposal:42]idNumTask:70
			$obRec.customerID:=[Proposal:42]customerID:1
			$obRec.tableNum:=Table:C252(->[Proposal:42])
			
			$obRec.title:=$atPath{Size of array:C274($atPath)}
			$obRec.description:=$obRec.title+"-"+String:C10($viCnt)+"-"+[Proposal:42]company:11
			$result_o:=$obRec.save()
		End for 
	...
	: ($obFormEvent.code=On Clicked:K2:4)
		If (Form:C1466.LBDocument_cur#Null:C1517)
			$vtPath:=Form:C1466.LBDocument_cur.path
