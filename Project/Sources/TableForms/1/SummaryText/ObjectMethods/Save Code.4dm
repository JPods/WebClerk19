...
Else 
	...
	If ((OK=1) & ($name#""))
		OK:=0
		If ($docName#"")
			$theFile:=Storage:C1525.folder.jitPrefPath+"P_"+$docName+".txt"
		...
		Else 
			...
			aiLoText2{$w}:=$theText
			aiLoText3{$w}:=""
			$theFile:=Storage:C1525.folder.jitPrefPath+"P_"+ailoText4{ailoText4}+".txt"
		...
		If (OK=1)
			...
			SEND PACKET:C103(myDoc; $name+"\t"+$theText+"\t"+"\r")
			CLOSE DOCUMENT:C267(myDoc)
		End if 
		//Tx_ListIn (Storage.folder.jitPrefPath+"P_"+aiLoText4{aiLoText4}+".txt")
