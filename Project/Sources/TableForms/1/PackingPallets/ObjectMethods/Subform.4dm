Case of 
	...
	: (ALProEvt=2)
		...
		KeyModifierCurrent
		If (OptKey=0)
			jSetFromArray(->[LoadItem:87]; ->aLiUniqueID; ->aLiLoadItemSelect; ->[LoadItem:87]idNum:16)
		Else 
			QUERY:C277([LoadItem:87]; [LoadItem:87]idNum:16=aLiUniqueID{aLiLoadItemSelect{1}})
		End if 
		ProcessTableOpen(Table:C252(->[LoadItem:87])*-1)
	: (ALProEvt=-1)
