...
Case of 
	: ($formEvent=On Double Clicked:K2:5)
		If (Record number:C243([ItemSiteBucket:136])>-1)
			ONE RECORD SELECT:C189([ItemSiteBucket:136])
			ProcessTableOpen(Table:C252(->[ItemSiteBucket:136])*-1)
	...
	: (($formEvent=On Load:K2:1) & (vHere<<>outLayoutTrigger))  //In header
		...
		End if 
		jsetInHeader(->[ItemSiteBucket:136])
		OLO_HereAndMenu
		//: ($formEvent=On Header)
