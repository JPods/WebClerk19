...
Case of 
	...
	: (iLoRecordNew)  //set in iLoProcedure only once, new record
		...
		[WorkOrder:66]dtCreated:44:=jDateTimeEnter
		If (<>vTempDate1#!00-00-00!)
			[WorkOrder:66]dtAction:5:=jDateTimeEnter(<>vTempDate1; <>vTempTime1)
		End if 
		If (<>vTempText1#"")
...
If ($doMore)  //action for the form regardless of new or existing record
	...
	// form calculations
	var $oDocs : Object
	$oDocs:=ds:C1482.Document.query("idNumTask  = :1 & idNumTask # 0"; [WorkOrder:66]idNumTask:22)
	// Add Documents
	...
	If ([WorkOrder:66]actionDate:105#!00-00-00!)
		[WorkOrder:66]dtAction:5:=jDateTimeEnter([WorkOrder:66]actionDate:105; [WorkOrder:66]actionTime:111)
	End if 
	...
	[WorkOrder:66]dtEndPlanned:69:=jDateTimeEnter([WorkOrder:66]dateBegin:106; [WorkOrder:66]timeBegin:109)  // assure alignment
	[WorkOrder:66]dtBeginPlanned:107:=jDateTimeEnter([WorkOrder:66]dateEnd:108; [WorkOrder:66]timeEnd:110)  // assure alignment
...
End if 
If (myOK=6)
	[WorkOrder:66]dtAction:5:=jDateTimeEnter(vDate5; vTime1)
	[WorkOrder:66]actionBy:8:=Current user:C182
	myOK:=0
End if 
