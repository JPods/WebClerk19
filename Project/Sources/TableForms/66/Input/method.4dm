...
Case of 
	...
	: (iLoRecordNew)  //set in iLoProcedure only once, new record
		...
		If (<>vTempDate1#!00-00-00!)
			[WorkOrder:66]dtAction:5:=DateTime_Enter(<>vTempDate1; <>vTempTime1)
		End if 
		If (<>vTempText1#"")
...
If ($doMore)  //action for the form regardless of new or existing record
	...
	[WorkOrder:66]dtEndPlanned:69:=DateTime_Enter([WorkOrder:66]dateBegin:106; [WorkOrder:66]timeBegin:109)  // assure alignment
	[WorkOrder:66]dtBeginPlanned:107:=DateTime_Enter([WorkOrder:66]dateEnd:108; [WorkOrder:66]timeEnd:110)  // assure alignment
...
End if 
If (myOK=6)
	[WorkOrder:66]dtAction:5:=DateTime_Enter(vDate5; vTime1)
	[WorkOrder:66]actionBy:8:=Current user:C182
	myOK:=0
End if 
