
var $event_o : Object
var $eventCode_i : Integer
$event_o:=FORM Event:C1606
$eventCode_i:=Form event code:C388

Case of 
	: ($eventCode_i=On Close Box:K2:21)
	: ($eventCode_i=On Load:K2:1)
	: ($eventCode_i=On Clicked:K2:4)
	: ($eventCode_i=On Double Clicked:K2:5)
		
	: ($eventCode_i=On Selection Change:K2:29)
		Form:C1466.form_o:=cs:C1710.cEditSubForm
		//Form.form_o:=cs.cEditSubForm.new("SF_Draft"; "Customer")
		Form:C1466.form_o:=cs:C1710.cEditSubForm.new("SF_Draft"; Form:C1466.dataClassName)
		Form:C1466.form_o.setLBFromCollection(Form:C1466.LB_Fields.sel.extract("fieldName"))
		LBX_SelectionIfEmpty(Form:C1466.form_o.lbName; Form:C1466.form_o.dataClassName)
		
		
		LB_UIDemo:=cs:C1710.listbox.new("LB_UIDemo"; ds:C1482["Customer"].all())
		var $coordinate_o : Object
		$coordinate_o:=LB_UIDemo.getCoordinates()
		
		
	: ($eventCode_i=On Begin Drag Over:K2:44)
		lbDragFrom_t:="lbFields"
		
	: ($eventCode_i=On Drag Over:K2:13)
		
	: ($eventCode_i=On Drop:K2:12)
		If (lbDragFrom_t="LB_Draft")
			
			
		End if 
End case 
