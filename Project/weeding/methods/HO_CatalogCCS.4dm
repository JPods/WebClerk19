...
Else 
	...
	If (OK=1)
		$vtVendorID:="CCS"
		$vtVendorTag:="-CCS"
		$vtPathFolder:="/Users/williamjames/Downloads/Copperfield"
...
If (OK=1)
	...
	$obCatalog.name:="CCS"
	var $viDTSync; $viDTinit : Integer  // define a DT for records in this import
	$viDTSync:=cs:C1710.cDateTime.new()
	var $dt : Integer
	
	
	// MustFixQQQZZZ: Bill James (2023-01-01T06:00:00Z)
	
	//$dt:=$viDTSync.getDT90()
	$obCatalog.dtSync:=111
	...
	For ($incFiles; 1; $cntFiles)
		...
		If ($w>0)  // skip if note a Table export
			...
			ConsoleLog("Incoming catalog: "+"\t"+String:C10($cntLines))
			...
			For ($incLines; 1; $cntLines)  // how many lines on a page
				...
				If ($raySize>0)
					Case of   // deal with each line
						: (($atCells{1}="") & ($incLines<6))  //some of the first lines are empty (($incLines=1) | ($incLines=3))
							// this is the date row
						: ($atCells{1}="@Price File As@")
							$vtSheetName:=$atCells{1}  // get the sheet name
							
						: ($cntCells<6)
							// drop out spread sheet has to F, 6 values for Copperfield
						: ($atCells{1}="Inventory ID")  // names of values
