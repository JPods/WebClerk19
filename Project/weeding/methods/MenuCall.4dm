...
If (Count parameters:C259=0)
	...
	$doMenu:=True:C214
	QUERY:C277([(Menu)]; [(Menu)]parentid=0)
	If (Records in selection:C76([(Menu)])>0)
		
		
		SELECTION TO ARRAY:C260([(Menu)]; $aMainRecordNum; [(Menu)]idNum; $aMainUIDs; [(Menu)]command; $aMainCmds; [(Menu)]target; $aMainTargets; [(Menu)]style; $aMainStyles)
		SORT ARRAY:C229($aMainCmds; $aMainUIDs; $aMainTargets; $aMainStyles)
		//      
		While ($myParentID>0)
			QUERY:C277([(Menu)]; [(Menu)]idNum=$myParentID)
			...
			$aPathStyles{1}:=[(Menu)]style
			$aPathTargets{1}:=[(Menu)]target
			$aPathCmds{1}:=Uppercase:C13([(Menu)]command)
			$aPathUIDs{1}:=[(Menu)]idNum
			$aPathRecordNum{1}:=Record number:C243([(Menu)])
			$myParentID:=[(Menu)]parentid
		End while 
		//    
...
Else 
	If ($recordID=0)  //if no command is given, find the primary set of [Menu]
		QUERY:C277([(Menu)]; [(Menu)]parentid=0; *)
		If ($rootType#"")
			QUERY:C277([(Menu)];  & [(Menu)]rootType=$rootType; *)  //if no command is given, find the primary set of [Menu]
		End if 
		QUERY:C277([(Menu)])
		SELECTION TO ARRAY:C260([(Menu)]; $aMainRecordNum; [(Menu)]idNum; $aMainUIDs; [(Menu)]command; $aMainCmds; [(Menu)]target; $aMainTargets; [(Menu)]style; $aMainStyles)
		...
		//End for 
	Else 
		QUERY:C277([(Menu)]; [(Menu)]idNum=$recordID)
		If (Records in selection:C76([(Menu)])=1)  //Set baseline
			...
			$aPathUIDs{1}:=[(Menu)]idNum
			$aPathRecordNum{1}:=Record number:C243([(Menu)])
			$findChildUID:=[(Menu)]idNum
			$myParentID:=[(Menu)]parentid
			$scriptText:=[(Menu)]script
			//
			//TRACE
			...
			QUERY:C277([(Menu)]; [(Menu)]parentid=$findChildUID)  //branch to child menus or items
			If (Records in selection:C76([(Menu)])=0)  //="Item")
				
				QUERY:C277([zzzWord:99]; [zzzWord:99]wordCombined:5=String:C10($findChildUID)+"MenuRef"; *)
				QUERY:C277([zzzWord:99];  & [zzzWord:99]tableNum:2=Table:C252(->[Item:4]))
				If (Records in selection:C76([zzzWord:99])>0)
					ARRAY TEXT:C222($relatedItems; 0)
					SELECTION TO ARRAY:C260([zzzWord:99]relatedAlpha:8; $relatedItems)
...
If ($doMenu)
	...
	If ($scriptText#"")
		ExecuteText(0; [(Menu)]script)
		$pageDo:="send text"
	Else 
		
		Case of 
			: ($format="table")
				menuList:="<Table style="+Txt_Quoted([(Menu)]style)+">"+"\r"  //put this on page to control values
				$k:=Size of array:C274($aMainCmds)
				//TRACE
				For ($i; 1; $k)
			...
			Else 
				menuList:="<ul class="+Txt_Quoted([(Menu)]style)+">"+"\r"  //put this on page to control values
				$k:=Size of array:C274($aMainCmds)
				//TRACE
				For ($i; 1; $k)
